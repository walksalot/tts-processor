...
class TextToSpeechProcessor:
    def __init__(self):
        ...
        if self.elevenlabs_available:
            try:
                ...
                # Create a list of voice IDs and names
                self.elevenlabs_voices = [
    ...
    def _load_anthropic_api_key(self) -> str:
        ...
        try:
            ...
            save_location = console.input("[bold yellow]Enter path to save API key (default: ~/.anthropic): [/bold yellow]") or "~/.anthropic"
            save_path = Path(os.path.expanduser(save_location))
            save_path.parent.mkdir(parents=True, exist_ok=True)
            save_path.write_text(api_key)
            console.print(f"[green]Saved API key to {save_path}[/green]")
        except Exception as e:
            logger.warning(f"Error saving Anthropic API key: {e}")
            console.print(f"[yellow]Could not save API key: {e}. Will use provided key for this session only.[/yellow]")
    ...
    def _load_elevenlabs_api_key(self) -> Optional[str]:
        """Load Eleven Labs API key from environment or ~/.elevenlabs file."""
        # First try environment variable
        ...
        if elevenlabs_file.exists():
            try:
                ...
                if re.match(r"^[a-zA-Z0-9]{32,}$", content):
                    logger.info("Loaded Eleven Labs API key from ~/.elevenlabs file")
                    return content
                
                # Try to find key in a key=value format
                match = re.search(r"(?:ELEVENLABS_API_KEY|api_key)[=:]\s*([^\s]+)", content)
                if match:
                    logger.info("Loaded Eleven Labs API key from ~/.elevenlabs file")
                    return match.group(1)
                ...
                try:
                    data = json.loads(content)
                    if isinstance(data, dict) and "api_key" in data:
                        logger.info("Loaded Eleven Labs API key from ~/.elevenlabs JSON file")
                        return data["api_key"]
                except json.JSONDecodeError:
                    pass
            except Exception as e:
                logger.warning(f"Error reading ~/.elevenlabs file: {e}")
        ...
        if ELEVENLABS_AVAILABLE:
            ...
            if Confirm.ask("Would you like to add an Eleven Labs API key?"):
                ...
                try:
                    ...
                    console.print("[green]Saved API key to ~/.elevenlabs file[/green]")
                except Exception as e:
    ...
    def display_voice_options(self, non_interactive=False) -> Tuple[str, str, float]:
        ...
        quality_table.add_column("Provider & Quality", style="cyan", width=20)
        ...
        quality_table.add_row(
            "1", "OpenAI Standard", "Regular quality, less expensive, faster processing"
        )
        quality_table.add_row(
            "2", "OpenAI HD", "Higher quality, more natural sounding, premium option"
        )
        
        # Add Eleven Labs if available
        if self.elevenlabs_available and "Eleven Labs voices" in self.voice_options:
            ...
            quality_table.add_row(
                elevenlabs_option, 
                "Eleven Labs Custom", 
                "High-quality custom voices, including your own voice if cloned"
            )

        console.print(quality_table)

        # If Eleven Labs is available, show a special note
        if self.elevenlabs_available and "Eleven Labs voices" in self.voice_options:
            console.print("\n[bold cyan]Note:[/bold cyan] Eleven Labs option includes your custom voice(s).")
        ...
        while format_choice not in ["1", "2", "3", "4"]:
            format_choice = console.input(
                "\n[bold yellow]Select output format (1/2/3/4): [/bold yellow]"
            )
    ...
    def process_chunk(
        ...
        try:
            ...
            if is_elevenlabs and self.elevenlabs_available:
                ...
                # Use OpenAI's TTS service with the highest quality settings available
    ...
    def text_to_speech(
        ...
        # Add note about HD quality
        hd_quality_note = ""
        if is_hd:
            # Different message based on format
            if output_file.endswith(".wav") or output_file.endswith(".flac"):
                hd_quality_note = f"\n[bold cyan]Note:[/bold cyan] HD quality audio with {output_file.split('.')[-1].upper()} format will provide the highest fidelity."
            else:
                hd_quality_note = f"\n[bold yellow]Note:[/bold yellow] For maximum HD quality, consider using WAV or FLAC format next time."
